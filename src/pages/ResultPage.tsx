...
const ResultPage = () => {
  ...
  const topicStats = useMemo<TopicStat[]>(() => {
    ...
    const grouped: Record<
      string,
      { name: string; qs: { q: Question; selected: number | null }[] }
    > = {};
    ...
    return Object.entries(grouped)
      .map(([, { name, qs }]) => {
        ...
        const correct = qs.filter(
          (x) => x.selected === x.q.correctAnswer,
        ).length;
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-card rounded-2xl border border-border p-6 sm:p-8 mb-6 shadow-sm">
          <div className="text-center mb-6">
            <div
              className={`inline-flex items-center justify-center w-28 h-28 rounded-full text-4xl font-bold font-mono-timer border-4 shadow-lg ${
                percentage >= 70
                  ? "border-success text-success shadow-success/20"
                  : percentage >= 40
                    ? "border-warning text-warning shadow-warning/20"
        ...
        {sectionStats && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div className="bg-card rounded-xl border border-border p-5">
              ...
              <div className="flex items-end gap-3">
                <span className="text-3xl font-bold font-mono-timer text-foreground">
                  {sectionStats.numerical.correct}/
                  {sectionStats.numerical.total}
                </span>
                <span className="text-sm text-muted-foreground mb-1">
                  (
                  {sectionStats.numerical.total > 0
                    ? Math.round(
                        (sectionStats.numerical.correct /
                          sectionStats.numerical.total) *
                          100,
              ...
              <div className="mt-3 h-2 rounded-full bg-muted overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${sectionStats.numerical.total > 0 ? (sectionStats.numerical.correct / sectionStats.numerical.total) * 100 : 0}%`,
                  }}
                />
              </div>
            </div>
            <div className="bg-card rounded-xl border border-border p-5">
              ...
              <div className="flex items-end gap-3">
                <span className="text-3xl font-bold font-mono-timer text-foreground">
                  {sectionStats.reasoning.correct}/
                  {sectionStats.reasoning.total}
                </span>
                <span className="text-sm text-muted-foreground mb-1">
                  (
                  {sectionStats.reasoning.total > 0
                    ? Math.round(
                        (sectionStats.reasoning.correct /
                          sectionStats.reasoning.total) *
                          100,
              ...
              <div className="mt-3 h-2 rounded-full bg-muted overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${sectionStats.reasoning.total > 0 ? (sectionStats.reasoning.correct / sectionStats.reasoning.total) * 100 : 0}%`,
                  }}
                />
              </div>
            </div>
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          <button
            onClick={() => navigate("/")}
            className="flex-1 min-w-[120px] px-4 py-3 bg-primary text-primary-foreground rounded-lg font-semibold hover:opacity-90"
          >
            Back to Topics
          </button>
          {result.isMockTest ? (
            <button
              ...
              className="flex-1 min-w-[120px] px-4 py-3 border border-border text-foreground rounded-lg font-semibold hover:bg-muted"
            >
              Retry Mock Test
            </button>
          ) : (
            <button
              onClick={() => navigate(`/test/${result.topicId}`)}
              className="flex-1 min-w-[120px] px-4 py-3 border border-border text-foreground rounded-lg font-semibold hover:bg-muted"
            >
              Retry Test
            </button>
          ...
          <button
            ...
            className="flex-1 min-w-[120px] px-4 py-3 bg-accent text-accent-foreground rounded-lg font-semibold hover:opacity-90"
          >
                  ...
                  <div key={t.topicName} className="bg-muted/50 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-3 text-xs shrink-0">
                        ...
                        <span
                          className={`font-bold ${t.accuracy >= 70 ? "text-success" : t.accuracy >= 40 ? "text-warning" : "text-destructive"}`}
                        >
                          {t.accuracy}%
                        </span>
                      </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center text-sm">
                ...
                <div className="bg-muted rounded-lg p-3">
                  <p className="text-lg font-bold text-foreground">
                    {topicStats.length > 0
                      ? Math.round(
                          topicStats.reduce((s, t) => s + t.accuracy, 0) /
                            topicStats.filter((t) => t.attempted > 0).length ||
                            1,
                        )
                      : 0}
                    %
