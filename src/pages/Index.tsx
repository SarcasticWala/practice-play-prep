      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
          ...
          {multiSelect && (
            <>
              ...
              <select
                ...
                className="px-3 py-2 rounded-xl text-sm font-semibold bg-card text-foreground border border-border focus:outline-none focus:ring-2 focus:ring-ring"
              >
                {[10, 20, 30, 50].map((n) => (
        ...
        <div className="bg-card rounded-2xl border border-border p-5 sm:p-6 mb-8 flex flex-wrap items-center justify-between gap-4 shadow-sm">
          <div className="flex gap-8">
            ...
            <div>
              <p className="text-2xl font-bold text-foreground">
                {Object.values(progress).reduce((s, p) => s + p.attempts, 0)}
              </p>
        ...
        {showBookmarks && (
          <div className="bg-card rounded-xl border border-border p-5 mb-6 fade-in">
            ...
            {bookmarks.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No bookmarked questions yet. Bookmark questions during tests for
                later review.
              </p>
            ) : (
                  ...
                  <div key={b.question.id} className="bg-muted rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-1">
                      {b.question.options.map((opt, j) => (
                        <span
                          key={j}
                          className={`text-xs px-2 py-1 rounded ${j === b.question.correctAnswer ? "bg-success/20 text-success font-bold" : "text-muted-foreground"}`}
                        >
